...
public class ResponsavelFamiliaTest {
	...
	@Test
	void deve_incluir_um_responsavel_familia() throws Exception {
		int quantitadeEsperado = 1;

		ResponsavelFamiliaRequestDTO responsavelFamiliaRequestDTO = new ResponsavelFamiliaRequestDTO("Maria",
				"maria@gmail.com", "021.365.012-15", "1998-08-15", Genero.FEMININO, EstadoCivil.SOLTEIRO);
		...
		List<ResponsavelFamilia> responsavelFamiliaRetornados = responsavelFamiliaRepository
				.findByNomeContainingIgnoreCase(responsavelFamiliaRequestDTO.getNome());
		Assertions.assertThat(responsavelFamiliaRetornados.size()).isEqualTo(quantitadeEsperado);
		Assertions.assertThat(responsavelFamiliaRetornados.stream().map(ResponsavelFamilia::getNome).toList())
				.contains(responsavelFamiliaRequestDTO.getNome());
	...
	void deve_buscar_um_responsavel_familia_pelo_id() throws Exception {
		...
		ResponsavelFamilia responsavelFamilia = ResponsavelFamilia.builder().nome("Flavio").email("flavio@gmail.com")
				.cpf("045.123.254-45").dataDeNascimento(dataDeNascimento).genero(Genero.MASCULINO)
				.estadoCivil(EstadoCivil.SOLTEIRO).build();
		responsavelFamiliaRepository.save(responsavelFamilia);

		MvcResult mvcResult = mockMvc.perform(get("/api/v1/responsavelFamilia/" + responsavelFamilia.getId()))
				.andReturn();
		...
		ResponsavelFamiliaResponseDTO criancaDTO = JsonUtil.mapFromJsonModuleJavaTime(content,
				ResponsavelFamiliaResponseDTO.class);

		Assertions.assertThat(responsavelFamilia.getId()).isEqualTo(criancaDTO.getId());
	}

	@Test
	void deve_remover_um_responsavel_familia_pelo_id() throws Exception {
		LocalDate dataDeNascimento = LocalDate.of(2002, 07, 19);
		ResponsavelFamilia responsavelFamilia = ResponsavelFamilia.builder().nome("Flavio").email("flavio@gmail.com")
				.cpf("045.123.254-45").dataDeNascimento(dataDeNascimento).genero(Genero.MASCULINO)
				.estadoCivil(EstadoCivil.SOLTEIRO).build();
		...
		List<ResponsavelFamilia> responsavelFamiliaRetornados = responsavelFamiliaRepository
				.findByNomeContainingIgnoreCase(responsavelFamilia.getNome());
